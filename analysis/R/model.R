...
#stan_mod multiple alpha discounts (e.g. multiple discounting coefficients for sensitivity analysis)
stan_mod_mult<-'
data
{
// [Current trial]
...
int<lower=1> num_htrials;


// discounting coefficient 
real alpha[num_htrials];  

// trial_ind: which historical trial 2=1st historical trial , 3=2nd historical trial....
int trial_ind[N_h];
...
{
//trial_ind=2,3,4
target+=alpha[trial_ind[j]-1]*bernoulli_lpmf(cog_h[j]| 1.0*exp(b)*c_h[j] / (1.0*exp(b)*c_h[j]+m_h[j]));
}
...
fit_mult<- stan_model(model_code=stan_mod_mult) #Need to run this line to use in function pp_sims


## Function: pp_sims_mult
#@dats: list of data sets current and historical (list of lists)
#@alpha: discounting coefficient in power prior (must have length of number of historical trials)
#@mean_init: initial prior in power prior's mean 
#@sd_init: initial prior in power prior's standard deviation
...
#multiple discounting coefficients

pp_sims_mult<-function(dats,alpha,mean_init,sd_init,true_bias,P,num_htrials)
{
  ## Results to index
  ...
  #b_h_hat: number cogs/ number of strieks ( historical trials-pooled )
  ...
  for(r in 1:length(dats$c_dats))
  {
    ...
    fit_mod <- sampling(
      ...
      data = list(
        ...
        mean_init = mean_init,
        sd_init = sd_init,
        num_htrials=num_htrials,
        trial_ind=h_dat$trial
      ),
      chains = 1,
      warmup = 1000,
      iter = 10000
...
#example=pp_sims(dats=temp,alpha=c(.1,.2,.3),mean_init=0,sd_init=2,true_bias=3,P=1,num_htrials=3)
...
#only one discounting coefficient
