...
abstract: |
  US law bars using peremptory strikes during jury selection because of prospective juror race, ethnicity, sex, or membership in certain other cognizable classes. Here, we extend a Bayesian approach for detecting such illegal bias. Current methods for detecting bias make implicit, but unacknowledged, assumptions about how to incorporate historical strike data into the bias detection process. Here, we show how to incorporate historical data on an attorney's use of peremptory strikes in past cases to estimate how likely that attorney has struck prospective jurors in a current trial because of their cognizable-class membership. In so doing, we use the power prior to adjust the weight of such historical information in the analysis. Using simulations, we show how the choice of alpha influences the estimation and detection of bias in the face of varying degrees of incompatibility between current and historical trial data. Finally, we extend this approach with a prototype software application (R Shiny application) to detect bias in peremptory-challenge in real time during jury-selection. We illustrate this application's use with real, historical strike data from a convenience sample of cases from one court.
...
In turn, the inference of bias based in part on historical strike data is relevant to lawyers for deciding whether to argue a _Batson_ violation, proving a prima facie case, and ultimately persuading the trial judge that bias in the particular case is more likely than not (Figure \@ref(fig:batsonworkflowfig)). Ignoring historical strike data increases the risk of underpowered or inflated estimates of attorney strike bias, because of the low number of strikes per trial (typically 6, 10, or 15 strikes per party). 

Relying on historical strike data, however, requires an approach to  dealing with incompatibility between current and historical trail data. Such incompatibility might arise due to missingness in the historical data or to the possibility that attorney's strike bias depends on certain trial-level characteristics that vary across trials (e.g., defendant race, charge severity). As a result, incompatibility might cause an attorney's bias parameter based on strike data for an attorney in the current trial to differ from such values based on historical strike data for that attorney.

Simply pooling the the strike data from current and past trials, and analyzing such data together, ignores such incompatibility, which can lead to inflated type I error and reduced power [@Bennett2020]. To increase power, one may use other methods, such as stratifying strike data by trial and applying the Cochran-Mantel-Haenszel (CMH) test [@Gastwirth2014, 295-297]. Such approaches, however, do not directly address incompatibility between current and historical strike data. For example, applying the CMH test requires first selecting only historical strike data from past trials that are "similar" to the current trial [@Gastwirth2014, 289, 297]. Assessing such similarity implies selecting the available historical strike data with respect to some attorney-bias-related set of trial-level characteristics that are present or absent in the current trial with the goal of reducing the incompatibility between current and historical data.

```{r batsonworkflowfig, fig.cap="Inference from Historical Strike Data at Key Steps of Batson Challenge", fig.align = 'center', out.width="100%", fig.pos = "h"}
...
In this paper, we extend a Bayesian approach to _Batson_ and similar challenges [@Kadane2021; @Kadane2018a; @Kadane2018b] to incorporate historical strike data in a manner that allows us to explicitly and transparently adjust for our assumptions about the degree of incompatibility between current and historical trials. In this approach, we use two steps. In step one, we specify a model of the peremptory-strike process in the court of interest that includes a bias parameter to which we assign an initial prior distribution. We use an attorney's strike data from past trials, as generated by that same peremptory-strike process, to estimate a posterior distribution for that bias parameter. We use the power prior [@Chen2000; @Ibrahim2015] to control how much the strike history from past cases affects the posterior distribution of the bias parameter. In step two, we estimate the posterior distribution of the bias parameter from the current case, using the posterior distribution developed with the historical information and the power prior. 

The use of the power prior in this context allows lawyers to explicitly control the assumed degree of incompatibility between the current and historical trial data. One approach is to fix the power prior parameter $\alpha$ and assess the sensitivity of results to different values [@Carvalho2021, 5252; @Ibrahim2015, 3734]. Specifically, lower values (closer to 0) of $\alpha$ should be selected if incompatibility is expected to be high. Higher values (closer to 1) of the power prior parameter should be selected if incompatibility is expected to be low. Accordingly, the lawyer bringing the _Batson_ challenge, after choosing the value of $\alpha$, must be able to defend that choice as appropriately corresponding to the likely degree of incompatibility between the current and historical strike data in the particular case. Since this overall procedure is to estimate bias to support the bringing of a _Batson_ challenge, the senstivity of bias detection to the value of $\alpha$ is used to evaluate sensitivity of the estimation of bias, not to indicate whether a _Batson_ challenge should succeed. 

Here, we demonstrate this approach with simulation studies where we vary the degree of incompatibility between current and historical trial data, and test our ability to detect bias using different values of alpha. Then, we present a software prototype that encodes the same approach with actual historical strike data. With that data, the prototype shows how attorneys and other can this approach to help them decide to argue, and ultimately prove, illegal bias in peremptory-challenge use in real time during jury selection.
...
To evaluate the proposed method, we conducted a simulation study using Stan `r rstan::stan_version()`, a Hamiltonian Monte Carlo engine for Bayesian inference by way of `r R.version$version.string` and RStan [@rstan]. <!--To measure convergence, we used trace plots, the Gelman-Rubin convergence diagnostic and Geweke’s diagnostics [@Gelman1992;@Gelman1998;@Geweke1992].--> These simulations primarily show how much the size of the historical data affects our ability to detect bias and how much the power prior discounted the weight of the historical data. As introduced above, $b > 0$ represents a prosecutor's bias against a prospective juror within the cognizable class, while $b = 0$ denotes no bias. For simplicity, we assume the defense attorney has no bias.
...
For the prototype app, we set two cognizable-class options: race and gender. This was because the standard juror questionnaire in this court did not ask about other possible cognizable classes. The data also did not expressly indicate the race or gender of the criminal defendant(s). For the levels of race and gender, we were limited by choices in the juror questionnaire (e.g., for gender, "male" and "female").

In the default setting, the pull-down menus for prosecutor and defense are set to "None". As a result, the app ignores any historical strike data and estimates the posterior distributions of the bias parameter for prosecutor and defense based only on the strike tally data and the initial prior ($b \sim N(0,2)$). Historical data will be included if the app user selects the name(s) of a prosection or defense attorney from the pull-down menu. Subsequently, the amount of historical data to be borrowed can be selected by the choice of $\alpha$. 
...
Now, the credible intervals clearly exclude zero. Thus, we have a credible basis to infer bias against racial-minority prospective jurors in how these attorney use their peremptory challenges in the present case. In the Appendix, we present additional illustrations wherein we use the app to detect gender bias and where we assign less weight to the historical strike data. 

# Discussion

Statistical methods for _Batson_ and similar challenges aim to help measure how much prospective jurors' race, gender, or other cognizable-class membership affected an attorney's use of peremptory strikes against them. These statistical methods do not aim to  prove illegal bias alone, but rather to lend support to an inference of bias and a subsequent _Batson_ challenge. The Bayesian approach presented here accounts for available historical strike data when estimating the posterior for the bias parameter. By incorporating the power prior, one can assess how sensitive that posterior estimate is to the available historical strike data. This is appealing because it makes assumptions about the incompatibility between current and historical trial data an explicit part of the process of identifying bias and the decision of whether to bring a _Batson_ challenge or not. 

Moreover, with a tool like our prototype app, attorneys can use the approach presented here to better detect strike bias for purposes of bringing or satisfying proof burden at different stages of _Batson_ and related challenges. The posterior distribution of the striking attorney's bias parameter can help the lawyer decide in real time whether to make a  _Batson_ challenge at all. If argued, the attorney can introduce that posterior distribution as relevant evidence, along with any other admissible evidence, for presenting a prima facie case and of ultimately proving illegal bias (Figure \@ref(fig:batsonworkflowfig)). 

To be sure, such evidence alone cannot account for the veracity of the striking attorney's proferred reasons. Besides, _Batson_ procedure allows any type of evidence, not just statistical evidence, for ultimately proving or disproving illegal bias in the particular case. Thus, as with any use of historical strike data, the approach here cannot always determine whether a _Batson_ challenge should succeed or fail. The approach here also requires more from those who would collect the appropriate historical strike data. For example, given the model we used, we required data on the cognizable-class composition of the prospective jurors not just at the time of the first strike, but at each time either party used a strike. 
...
In cases where lawyers contest the degree of compatibility, those lawyers can and should interrogate the choice of $\alpha$ by selecting multiple $\alpha$ values and observing how sensitive the posterior distribution of the striking attorney's bias parameter is to the choice of $\alpha$ value. (Our protoype app permits this kind of testing of sensitivity in real time by providing multiple options for $\alpha$ values.) If the inference of bias is sensitive to the choice of $\alpha$ values, all else equal, then worries about incompatibility are warranted. As a result, the historical strike data, without more information, helps less to prove a _Batson_ violation than it would otherwise would. 

By their choice of $\alpha$, the lawyer explicitly and transparently denotes the degree of weight placed on the historical strike data in the analysis. This transparency matters, because it thwarts lawyer efforts to strategically select $\alpha$ values to prove or disprove illegal strike bias. Rather, at the key steps of the _Batson_ challenge process (Figure \@ref(fig:batsonworkflowfig)), the lawyers on both sides of a _Batson_ challenge know that the lawyers on the other side can always directly and repeatedly assess how sensitive any bias estimation is to the choice of $\alpha$. In any case, the choice of $\alpha$ cannot always determine whether a trial judge finds that illegal bias is more likely than not (e.g., a _Batson_ violation), because no approach to estimating bias based on historical strike data has that ability. 

In future work we hope to examine data-driven approaches to selecting $\alpha$; where the degree of incompatibility can be estimated and used to inform the choice of the degree of discounting to apply to historical data. Knowing how to quantify incompatibility and how to specify an appropriate prior for $\alpha$ is an area of active research [e.g., @Pawel2022]. In any case, any formulation for estimating $\alpha$ based on incompabitiliy between current and historical trials would require justification in court (or by practitioners). 
...
Here, we extend the discussion in section \@ref(prototype) of the paper by discussing the data collection and presenting additional illustrations of how a lawyer might use the prototype app. 
