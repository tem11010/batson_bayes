...
#    orcid_id: 0000-0001-7387-1307
  - Xiaomeng Li, University of Connecticut | Ernest & Young
  - Timothy E. Moore, University of Connecticut
#    orcid_id: 0000-0002-9576-0517
#output: distill::distill_article
output: pdf_document
bibliography: references.bib
abstract: |
  US constitutional law bars using peremptory strikes during jury selection because of race, gender, or another cognizable classes. Here, we propose a Bayesian approach for detecting such illegal bias based on an attorney's pattern of peremptory challenges in both a current trial as well that attorney's use of strikes in past cases. Specifically, we show how to use historical data on an attorney's use of peremptory strikes in past trials to estimate, in a current trial, how likely an attorney has used peremptory challenges because of the cognizable class of prospective jurors. We also introduce the use of a _power prior_ to provide flexibility in the use of such historical data. We show the utility of our model on simulated data. Finally, we extend our approach by presenting an R Shiny application that can be used to examine jury selection data in real time. We demonstrate the app using actual jury-selection data from a convenience sample of criminal trials in the federal district court of Connecticut from YYYY to YYYY.
...
The posterior distribution does not have a closed form.  Accordingly, we used the Metropolis-Hasting algorithm to sample $b$ from the posterior distribution. To measure convergence, we used traceplots, the Gelman-Rubin convergence diagnostic [@Gelman1992; @Gelman1998] and Geweke’s diagnostics [@Geweke1992]. 
...
Following this suggestion, we describe here a prototype software application ("app") that implements the approach described above using `r R.version$version.string`[@R] and shiny [@shiny] and comes loaded with real jury-selection data from a convenience sample of criminal cases in the federal district court for Connecticut during YYYY through YYYY. To partially de-identify this historical data, we excluded defendants' names and substituted fictitious names for the attorneys using @charlatan.  

With the app, users can input the current information on the use of peremptory strikes as shown in table \@ref(tab:inpts), and then select the name of the prosecutor and the defendant from the pull-down window, if available, in the historical data. If an attorney's name cannot be found, it means that the app does not have any historical information on that attorney's use of peremptory strikes.

The main screen displays two graphs -- one for the prosecution and the defense - that displays the prior and posterior density plots for the bias parameter.

```{r inpts, echo=FALSE, eval = FALSE}
...
Sachin S. Pandya: Conceptualization, Investigation, Software, Writing; Xiaomeng Li: Conceptualization, Formal Analysis, Software, Writing - original draft; Timothy Moore: Conceptualization, Supervision, Visualization, Writing. Based on categories in https://casrai.org/credit/].
