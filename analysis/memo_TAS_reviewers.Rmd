...
5. Choosing a credible interval amounts to deciding how conservative one wishes to be about inferring bias from past strike data, because we define bias "detection" narrowly to mean when the posterior for the bias parameter falls outside the chosen credible interval. Thus, the larger the credible interval one chooses, the greater the chance that zero falls within it. For clarity, we now focus on one credible interval (95%) in the paper and have placed simulation results based on 90% and 80% credible intervals in the Appendix. 
...
We added this information to the paper (sec. 2.5, p. 18).
...
Thank you for this reference. The paper now cites to the model in Gastwirth (2005). On "data from an actual case," section 2.5 of the paper (our discussion about the prototype software application) expressly relies on actual strike data from a sample of criminal cases. As mentioned above, we intend the reader to gain understanding by following along using our prototype software application.
