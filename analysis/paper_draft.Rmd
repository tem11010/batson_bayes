---
title: "Batson Challenges Using Historical Data"
date: "`r Sys.Date()`"
...
This paper shows how to use historical jury-selection data to estimate how likely an attorney has used peremptory challenges to strike potential jurors because of juror race or gender in violation of _Batson v. Kentucky_ (1986), and its progeny.

In the US, once a group of individuals selected for jury-service appear in court as scheduled, this group is questioned by the parties' attorneys and the trial judge.  During this "voir dire" process, a prospective juror, if not selected for the jury or excused by the trial judge for cause, may be dismissed if a party's attorney uses a peremptory challenge against them. 
...
By law, evidence supporting a _Batson_ challenge may include not only the circumstances of the jury selection itself, but also data on the pattern of peremptory challenges in past cases. For example, in a _Batson_ challenge against a prosecutor in a criminal case, such historical data would be relevant to whether (1) to conclude that the defendant has a prima facie case; (2) to believe a prosecutor from that office who has offered a race-neutral explanation for striking, say, an African-American juror in that case; and (3) to ultimately accept or reject a Batson challenge in that case [e.g., Miller-El v. Dretke, 545 U.S. 231, 253 (2005); Riley v. Taylor, 277 F.3d 261, 280-81, 283 (3d Cir. 2001); Cochran v. Herring, 43 F.3d 1404, 1412 (11th Cir. 1995)]. The US Supreme Court recently identified the kinds of evidence to support _Batson_ challenge as including "relevant history of the State's peremptory strikes in past cases" [e.g, Flowers v. Mississippi, 139 S. Ct. 2228, 2243 (2019)].

Prior work on statistical methods for _Batson_ challenges, however, have largely focused on strikes used in a single case taken alone. For example, @Barrett2007 modeled bias detection for _Batson_ challenges, given a "struck-jury" procedure in which the parties took turns striking prospective jurors. @Gastwirth2016 applied Fisher's exact test to detect bias in the pattern of strikes in each of ten published cases. For each of three cases, @Kadane2018 compared various frequentist approaches (including Fisher's exact test) with a Bayesian approach.

When considering strikes in multiple trials in the same State or judicial district, @Gastwirth2013 criticized the use of a paired t-test to compare the differences in strike rates of Black and non-Black venire members in the aggregate. Rather, both they and @Gastwirth2014 argued for applying the Cochran–Mantel–Haenszel test by taking the difference of the sum of the observed numbers of Black venire members challenged in each trial and the corresponding sum of the expected numbers of Blacks challenged, and then comparing it to the standard deviation of the difference.

Moreover, drawing inferences from peremptory challenges in past cases is hard, because jury-selection records are often not readily available and largely on paper, not electronic or machine-readable formats [@Grosso2017; @Wright2018], and otherwise require painstaking efforts to collect [e.g., @Craft2018].

This may matter for how much weight to assign such historical data if data on the use of strikes in past trials is missing not at random. For example, for any given attorney, there may be only incomplete information on that attorney's use of strikes in previous trials in a particular State or court. Possible reasons include limited access the relevant records (e.g., jury-selection records kept by a jury administrator, not in a case file) and the expense of obtaining copies of them.  Moreover, because some standard jury-selection forms do not require recording relevant juror information, such as juror race or gender, lawyers and researchers must turn to other data sources (e.g., voter registration rolls) with that information. Finally, regardless of what is supposed to be recorded, judges and court staff vary in how often they actually fill in standard forms with the required information.
...
Here, we extend the Bayesian approach to _Batson_ challenges to assess whether one can credibly infer illegal bias based solely on an attorney's pattern of peremptory challenges using both present strike data and historical data that attorney's strikes in past cases. 

In short, our workflow is this: First, specify a model of the peremptory-strike process that best approximates the peremptory-strike process in the jurisdiction of interest. Second, assign a prior distribution to the bias parameter $b$ that reflects the legal requirement that the _Batson_ challenger bear the burden of proof. Here, we assume a normal prior for $b$ with mean 0 and standard deviation 2. Third, using the strike data in past trials generated by the same peremptory-strike process, estimate a posterior distribution for $b$.  Finally, using data on the target attorney's strikes in the present case, use that posterior distribution as the prior for $b$ and update. 

In so doing, we make two main contributions. First, we use the power prior to control how much the strike history in past cases affects the posterior distribution of the bias parameter $b$. The power prior is a way to create informative prior distributions based on historical data so as to control how much historical data affects the posterior distribution of the unobserved variables of interest. The key insight is to raise the likelihood function based on the historical data to a power parameter [@Ibrahim2000; @Ibrahim2015].
...
Following @Kadane2018 and @Barrett2007, we model a struck-jury peremptory-challenge process as follows. 
...
We also generated historical strike data of three sizes (same, double, and triple the size of current data) as well as seven different values of the bias parameter for generating the historical data ("historical" $b = {-3,-2,-1,0,1,2,3}$). 

We let historical $b$ differ from current $b$ (our parameter of interest) for two reasons. First, it allows us to simulate noise associated with historical strike data that is incomplete not at random. Second, it could also simulate a real difference in the bias parameter that depends on a feature that the current trial has but that _all_ the past trials in the historical data do not. To illustrate, assume a prosecutor with two true bias values: $b > 0$ if the criminal defendant is Black, $b = 0$ if not. If our historical data contains only strike data in cases with non-Black defendants, then that prosecutor's historical $b$ will differ from the current case with a Black defendant.

By using the power prior, we can control the weight that we put on historical data by modifying the weight parameter $a$. If $a = 1$, the historical strike data is equally weighted with the present data on strikes. If $a < 1$, the historical data matters proportionally less. As discussed above, if some of the historical data is missing not at random, we may wish to assign $a < 1$ to evaluate how sensitive the posterior of the bias parameter to the historical data on strikes. Accordingly, in the simulation study, we tried different values of the weight parameter $a = (0.1,0.3,0.5,0.7,1)$ to show how that parameter affected modeling accuracy. 
