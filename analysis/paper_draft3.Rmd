...
```{r batsonworkflowfig, fig.cap="Batson workflow", fig.align = 'center', out.width="50%", fig.pos = "h"}

load_path <- here::here("figures","batson_workflow_fig.png")
knitr::include_graphics(load_path)


```

A _Batson_ challenge typically proceeds in a three-step sequence (Figure \@ref(fig:batsonworkflowfig)). First, the challenger must meet an initial burden (the _prima facie_ case) of producing just enough evidence of strike bias to raise a sufficient initial inference of illegal bias. If so, then at step two, the striking attorney must produce (permissible) reasons for their challenged strikes. Third, the challenger must discredit those proffered reasons and ultimately persuade the trial judge to find that it is more likely than not that the striking attorney acted with illegal bias.
...
Moreover, with a tool like our prototype app, attorneys can use the approach here in to better detect strike bias for _Batson_ and related challenges, in three ways. First, the posterior distribution of the bias parameter can help the lawyer decide in real time whether it is worth raising a  _Batson_ challenge at all.

Second, if raised, the tool can help the _Batson_ challenger meet its evidentiary burdens at steps one and three of a _Batson_ challenge (Figure \@ref(fig:batsonworkflowfig)). To meets its evidentiary burdens at the first and third steps, the attorney can introduce the posterior distribution of the striking attorney's bias parameter as relevant statistical evidence. In this way, the tool helps detect bias. But it in no way suffices for deciding a _Batson_ challenge. Such statistical evidence alone cannot account for the veracity of the striking attorney's reasons offered at step two. Besides, _Batson_ procedure allows for any type of circumstantial evidence of bias at steps one and three.
...
At the same time, US law currently affords an avenue for reducing this missing data problem by systematically collecting historical strike data prospectively. A criminal defendant has a Sixth Amendment right to a public trial that presumptively precludes excluding the public from the voir dire of prospective jurors. Similarly, the public has a presumptive First Amendment right to attend jury voir dire in criminal cases, even if prosecutor and defendant agree to exclude the public [@press1984; @presley2010]. Upon exercise of these rights, courtroom observors can attend jury voir dire and collect relevant strike data based on what they observe in real time.  Even if historical strike data is hard to collect retrospectively, courtroom observers can over time build up a dataset for strike data that they would have been less likely to obtain from court records after the fact.
...
In practice, the anticipated degree of incompatibility should influence which value of $\alpha$ to select. For example, given perfect compatibility ($\lvert b_{curr} - b_{hist} \rvert = 0$), we should let $\alpha = 1$ (equal weight between current and historical data). This will maximize bias detection even when the number of strikes per trial is small. This effect of increased bias detection can be seen in the simulation results depicted in Figures \@ref(fig:figbd9515), \@ref(fig:figbd9510), and \@ref(fig:figbd956) along the positive diagonals of the individual blocks.
...
More importantly, in cases where lawyers contest the degree of compatibility, those lawyers should interrogate the choice of $\alpha$ by selecting multiple $\alpha$ values and observing how sensitive the posterior distribution of the striking attorney's bias parameter is to the choice of $\alpha$ value. (Our protoype app permits this kind of testing of sensitivity in real time by providing multiple options for $\alpha$ values.) If the inference of bias is sensitive to the choice of $\alpha$ values, all else equal, then worries about compatibility are warranted and the historical strike data, without more, is less helpful in steps one and three of the _Batson_ procedure than it would otherwise would be. One virtue of this approach is that the choice of $\alpha$ explicitly denotes the degree of weight placed on the historical strike data in the analysis.
