...
We have revised accordingly. [TBD]
...
As we discuss in the paper (page X, lines YY-YY), how one chooses the subsetting criteria (e.g., "similar" cases) would have to be justified with respect to its _assumed_ effect on the degree of compatability between the historical trials and the current trial ($b_hist - $b_curr$). We stress _assumed_, because outside simulations like ours, neither $b_hist$ nor $b_curr$ are directly observable in actual cases. Accordingly, for any subset of the historical strike data one chooses, our guidance for choosing an $\alpha$ value remains the same: Check for how sensitive to the posterior estimate of strike bias to different values of $\alpha$.  The current version of the software prototype does not contain a feaure that lets users subset the historical data by some characteristic (e.g., date, defendant race). The app, however, in principle, could be modified to add that kind of subsetting feature as a step prior to estimating the posterior distribution for bias.
...
Thank you for this comment.  We have revised the paper accordingly.
