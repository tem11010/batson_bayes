...
Relying on historical strike data, however, requires an approach to missingness in the historical data or to the possibility that attorney's strike bias depends on certain trial-level characteristics that vary across trials (e.g., defendant race, charge severity). As a result, there may be incompatibility between plausible values of any attorney's bias parameter based on strike data for an attorney in the current trial and such values based on historical strike data for that attorney.
