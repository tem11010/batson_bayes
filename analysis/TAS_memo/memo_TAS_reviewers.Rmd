...
While it is true, based on our simulations, that bias can be detected even if there is no bias in the current trial's strike pattern under certain conditions: if there are a sufficient number of historical cases (e.g., n=3) and if bias in the historical trials is sufficiently strong (i.e., strong incompatibilty). Contrary to this comment, this is exactly why we advocate for the use of $\alpha$! In scenarios like the one just described (n=3, strong incompatibility), sensitivity of bias detection can be evaluated by changing alpha and noting its effect on the estimation of bias.
...
[Maybe mention that these issues - low power, type I error, etc are not unique to our method?]
...
[Mention that measuring incomaptibility and choice of a $\alpha$ in face of incompatibility is an active area of research (refs) with no clear solution. Our providing range of alpha values so that users can evaluate sensitivity is common practice (carvallo & ibrahim 2021)]
