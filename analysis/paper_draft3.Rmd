...
In turn, the inference of bias based in part on historical strike data is relevant to lawyers at three of the four steps of the _Batson_ challenge, i.e., for deciding whether to argue a _Batson_ violation, for establishing a prima facie case, and for ultimately proving that attorney strike bias in the particular case is more likely than not (Figure \@ref(fig:batsonworkflowfig)).


```{r batsonworkflowfig, fig.cap="Batson workflow", fig.align = 'center', out.width="100%", fig.pos = "h"}
...
Second, if raised, the tool can help the _Batson_ challenger meet the  evidentiary burdens of presenting a prima facie case and of ultimately proving illegal bias (Figure \@ref(fig:batsonworkflowfig)). To meet those burdens, the attorney can introduce the posterior distribution of the striking attorney's bias parameter as relevant statistical evidence. In this way, the tool helps detect bias. 

But as with _any_ inference from historical strike data, the approach here in no way suffices for deciding a _Batson_ challenge. Such statistical evidence alone cannot account for the veracity of the striking attorney's proferred reasons. Besides, _Batson_ procedure allows for any type of circumstantial evidence of bias for purposes of the ultimately proving or disproving illegal bias.
